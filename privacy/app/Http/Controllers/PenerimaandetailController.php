...
class PenerimaandetailController extends Controller
{
    ...
    public function isipart()
    {
        ...
        //vendor code
        if (strlen($pembelian->kode_vendor) > 2) {
        ...
        $cek_part = PenerimaanDetail::on($konek)->where(DB::raw('LEFT(partnumber,16)'),$part1)->orderBy('id','desc')->first();
        
        if ($cek_part != null){
            $parto = substr($cek_part->partnumber,0,16);
            if ($part1 == $parto){
                $kode = substr($cek_part->partnumber,16,3);
                $kode2 = 0;
                $kode3 = 0;
                $kode += 1;
                if ($kode >= 100){
    ...
    public function checkpart()
    {
        ...
        if($cek_serial == null){
            $message = [
                'success' => false,
                'title' => 'Gagal',
                'message' => 'Nomor Seria Number Lama Tidak Terdaftar.'
            ];
            return response()->json($message);
        }
    }
