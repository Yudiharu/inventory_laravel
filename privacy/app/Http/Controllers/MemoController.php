...
class MemoController extends Controller
{
    ...
    {
        $konek = self::konek();
        $memo = Memo::on($konek)->find($request->no_pembelian);
        $vendor = Vendor::find($request->kode_vendor);

        $memodetail = MemoDetail::on($konek)->where('no_pembelian', $request->no_pembelian)->get();
        $tanggal = $request->tanggal_pembelian;

        $Memo = Memo::on($konek)->find($request->no_pembelian)->update($request->all());
