...
class RequestpembelianController extends Controller
{
    ...
    {
        $konek = self::konek();
        $create_url = route('requestpembelian.create');
        ...
        return view('admin.requestpembelian.index',compact('create_url','Company','period', 'nama_lokasi','nama_company','Lokasi'));
    ...
    {   
        $konek = self::konek();
        return Datatables::of(Requestpembelian::on($konek))->make(true);
    }
    ...
    {
        $konek = self::konek();
        $header = Requestpembelian::on($konek)->find(request()->id);
        ...
        $message = [
            'success' => true,
            'title' => 'Simpan',
            'message' => 'Request '. $header->no_request. 'telah di close.'
        ];
        return response()->json($message);
    }
    ...
    public function exportPDF(){
        ...
        return $pdf->stream('Laporan Pembelian '.$request.'.pdf');
    }
    ...
    {
        $konek = self::konek();
        $requestbeli = Requestpembelian::on($konek)->find($pembelian);
        $tanggal = $requestbeli->tgl_request;
        $no_request = $requestbeli->no_request;

        // $memodetail = MemoDetail::on($konek)->with('produk')->where('no_memo', $requestbeli->no_memo)
        // ->orderBy('created_at','desc')->get();

        $memo = Memo::on($konek)->where('status','POSTED')->pluck('no_memo','no_memo');

        $list_url= route('requestpembelian.index');
        ...
        return view('admin.requestpembeliandetail.index', compact('memo','requestbeli','list_url','Produk','period','nama_lokasi','nama_company'));
    ...
    {
        $konek = self::konek();
        $reqproduketail= RequestProduk::on($konek)->with('produk')->where('no_request',request()->id)
        ->orderBy('created_at', 'desc')->get();

        $requestpembelian= Requestpembelian::on($konek)->where('no_request',request()->id)->first();
        
        $output = array();

        if($reqproduketail){
                foreach($reqproduketail as $row)
                {
                    $output[] = array(
    ...
    {
        $konek = self::konek();
        $req = Requestpembelian::on($konek)->find(request()->id);
        $req->status = 'POSTED';
        $req->save();

        $reqdetail = RequestpembelianDetail::on($konek)->where('no_request',request()->id)->get();
        foreach ($reqdetail as $row){
            $memo = Memo::on($konek)->where('no_memo',$row->no_memo)->first();
        ...
        $message = [
            'success' => true,
            'title' => 'Simpan',
            'message' => $req->no_request.' berhasil di POST',
        ];
        return response()->json($message);
    ...
    public function Unpost()
    {
        ...
        foreach ($reqdetail as $row){
            $memo = Memo::on($konek)->where('no_memo',$row->no_memo)->first();
            $cektrf = Transfer::on($konek)->where('no_memo',$row->no_memo)->where('status','<>','OPEN')->first();
        ...
        $message = [
            'success' => true,
            'title' => 'Simpan',
            'message' => $req->no_request.' berhasil di UNPOST',
        ];
        return response()->json($message);
    }
    ...
    {
        $konek = self::konek();
        $tanggal = $request->tgl_request;
        $pembelian = Requestpembelian::on($konek)->create($request->all());

        $no = Requestpembelian::on($konek)->orderBy('created_at','desc')->first();
        $nama = auth()->user()->name;
        $tmp = ['nama' => $nama,'aksi' => 'Simpan No. Request: '.$no->no_request.'.','created_by'=>$nama,'updated_by'=>$nama];
        //dd($tmp);
        user_history::on($konek)->create($tmp);
    ...
    {
        $konek = self::konek();
        $no_request = request()->id;
        $data = Requestpembelian::on($konek)->find($no_request);
        $status = $data->status;
    ...
    public function updateAjax(Request $request)
    {
        ...
        $requestpembelian = Requestpembelian::on($konek)->find($no_request);
        Requestpembelian::on($konek)->find($no_request)->update($request->all());

        $nama = auth()->user()->name;
        $tmp = ['nama' => $nama,'aksi' => 'Edit No. Request: '.$no_request.'.','created_by'=>$nama,'updated_by'=>$nama];
                //dd($tmp);
        user_history::on($konek)->create($tmp);
    ...
    public function hapus_request()
    {
        ...
        $no_request = request()->id;
        $data = Requestpembelian::on($konek)->find($no_request);
        $tanggal = $data->tgl_requestl;

        $cek_detail = RequestpembelianDetail::on($konek)->where('no_request',$no_request)->first();
        if($cek_detail == null){
            $status = $data->status;
            ...
            if($status == 'OPEN'){
                ...
                $nama = auth()->user()->name;
                $tmp = ['nama' => $nama,'aksi' => 'Hapus No. Request: '.$data->no_request.'.','created_by'=>$nama,'updated_by'=>$nama];
                        //dd($tmp);
                user_history::on($konek)->create($tmp);
                ...
                $message = [
                    'success' => true,
                    'title' => 'Update',
                    'message' => 'Data ['.$data->no_request.'] telah dihapus.'
                ];
                return response()->json($message);
            }
        }
