...
<head><meta http-equiv="Content-Type" content="text/html; charset=utf-8">
    ...
    ?>
    <?php if ($aset != 'SEMUA') { ?>
        <title>LAPORAN DATA PEMAKAIAN ASET <?php echo e($aset); ?></title>
    <?php } else { ?>
...
<div class="fixed-header">
        ...
        <br><br>
            <?php
                if ($kategori != 'SEMUA') {?>
                <?php if ($aset != 'SEMUA') { ?>
                    <h1>LAPORAN PEMAKAIAN MOBIL KATEGORI <?php echo $kategori; ?> NO ASET <?php echo $aset; ?></h1>
                <?php }else { ?>
                    <h1>LAPORAN PEMAKAIAN MOBIL KATEGORI <?php echo $kategori; ?></h1>
                <?php } ?>
            <?php } 
                else{?>
                <?php if ($aset != 'SEMUA'){ ?>
                    <h1>LAPORAN PEMAKAIAN MOBIL NO ASET <?php echo $aset; ?></h1>
                <?php } else { ?>
                    <h1>LAPORAN PEMAKAIAN MOBIL</h1>
                <?php } ?>
            <?php } ?>
...
?>
    <table class="grid1" style="margin-bottom: 25px;width: 100%; font-size: 11px">
        ...
        <?php foreach ($pemakaiandetail as $key => $row) : ?>
        <tbody>
        ?>
            <tr class="border">
            ...
            <?php } ?>
                ...
                <td class="border" align="right"><?php echo number_format($row->harga * $row->qty,'0',',','.') ?></td>
                <?php endif; // app('laratrust')->can ?>
                <?php
                    if ($lokasi == 'SEMUA') {?>
